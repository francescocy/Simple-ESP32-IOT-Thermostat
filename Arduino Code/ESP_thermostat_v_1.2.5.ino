/*
 * New in release 1.2.5:
 * Resolved a bug causing the thermostat not to enable when timer is programmed via html
...
String input_field2prev = "true"; //Support variable to enable automatic Timer enable/disable logic
...
String firmware = "V1.2.5 08/02/22";
int caldaia = 0;
...
<!DOCTYPE HTML><html><head>
  ...
  <a href='https://cloud.lilik.it/s/y3zB48q2M7eSgWN'>&#128209;Guida al termostato</a><br><br>
  <a href='https://github.com/francescocy/Simple-ESP32-IOT-Thermostat'>&#128049;GitHub&#128025;</a><br><br>
...
void drawMenu() {
    // Fill LCD with black and Print main menu elements
...
gfx->begin();
//Filling with black background
gfx->fillScreen(BLACK);

//setting the font & printing boot-up welcome message
...
sensors.begin();
  ...
  server.on("/get", HTTP_GET, [] (AsyncWebServerRequest *request) {
    ...
    if (request->hasParam(input_paramter3)) {
      ...
      //Enable thermostat when timer is started
...
// Reset if I/O & Timer are pressed at the same time
...
//Hardware Buttons Input Logic
  ...
  else if(digitalRead(BTN2) == LOW && timer >= 361 && del == 0){
    timer = 0;
    BTN_Press = 1;
    del = btndelays;
  }
  // New auto-disabling timer logic, prevents automatic disabling on every cycle, only disables when enable option is unchecked
  if (input_field2 != input_field2prev && input_field2 != "true") {
    timer = 1/(120*tscale); // Reset timer when disabling
  }
  input_field2prev = input_field2;
...
//Static elements are printed only once during setup and are not repeated here, this helps reduce flicker
